...
class EvaluatorBase(Callback):
    ...
    def get_input_tensor(self, input_desc, min_batch_size = 1, **kwargs):
        if self._tensor_io: return self._tensor_io.getInputTensors()

        self._input_desc = input_desc
        kwargs = kwargs.copy()
        kwargs.update(self._kwargs)
        self._tensor_io = self._data_io.getTensorIO(self._pool_name + '/pred', input_desc, queue_size = 0, is_training = self._is_training, min_batch_size = min_batch_size, **kwargs)

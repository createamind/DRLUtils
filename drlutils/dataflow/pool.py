...
class AgentPoolBase(DataPool.Pool):
    def __init__(self, name, size, isTrain,
                 maxAgentsPerProcessor = 256,
                 rpc_bind = ('127.0.0.1', int(os.environ['ICE_RPCIO_LISTEN_PORT_BASE']) + 30),
                 dio_addr = ('127.0.0.1', 0),
                 ds_addr = ('127.0.0.1', int(os.environ['ICE_RPCIO_LISTEN_PORT_BASE']) + 40),
                 **kwargs):
        self._name = name
        assert(size > 0)
        self._size = size
        self._isTrain = isTrain
        if maxAgentsPerProcessor < 0: maxAgentsPerProcessor = size
        self._maxAgentsPerProcessor = min(maxAgentsPerProcessor, size)
        ...
        logger.info("AgentPool {} create, isTrain={}, size={}, Agent cls={}"
                    .format(self._name, self._isTrain, self._size,
...
class AgentPoolNoBatchMode(AgentPoolBase):
    ...
    def _createAgent(self, agentIdent):
        assert(self._cls_agent)
        kwargs = self._kwargs.copy()
        kwargs.update({'isTrain': self._isTrain,
                       'dio_addr': self._dio_addr,
                       'dio_name': self._name,
                       })
        return self._cls_agent(agentIdent, **kwargs)

    def run(self):
        logger.info("[{}]: AgentPool batch mode start running".format(self._name))
        import multiprocessing as mp
        procs = []
        for agentIdent in range(self._size):
